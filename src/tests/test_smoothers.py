...
"""
Unit tests for statistical smoothing functions and associated interval calculations.

This test module ensures the correct behavior of statistical smoothing methods implemented in 
`src/statistics/smoothers.py`, including LOWESS, exponential smoothing, and Simple Moving Average (SMA), 
as well as their respective confidence and prediction intervals.

Tests cover:
- Basic functionality (output shapes, NaN checks)
- Confidence and prediction interval validity
- Edge cases (short series, constant series)

Run these tests from the project root with:
>>> python -m src.tests.test_smoothers
"""
...
def test_lowess_ci_pi():
    ...
    # Ensure intervals have correct shape
    for arr in [ci_lower, ci_upper, pi_lower, pi_upper]:
        assert arr.shape == fitted.shape, "Interval arrays must match fitted values shape."
...
def test_exp_smooth_ci_pi():
    ...
    # Check interval shape matches fitted values
    for arr in [ci_lower, ci_upper, pi_lower, pi_upper]:
        assert arr.shape == fitted.shape, "Interval arrays must match fitted values shape."
...
def test_short_series():
    ...
    assert len(exp_out) == len(short_series), "Exponential smoother fails on short series."
...
def test_constant_series():
    ...
    for func in [lowess_ci_pi, exp_smooth_ci_pi, sma_ci_pi]:
        ...
        assert np.allclose(fitted, 5), f"{func.__name__} not stable for constant series."
        assert np.allclose(ci_lower, fitted), f"{func.__name__} CI lower bound mismatch on constant series."
        assert np.allclose(ci_upper, fitted), f"{func.__name__} CI upper bound mismatch on constant series."
        assert np.allclose(pi_lower, fitted), f"{func.__name__} PI lower bound mismatch on constant series."
        assert np.allclose(pi_upper, fitted), f"{func.__name__} PI upper bound mismatch on constant series."
...
def test_invalid_inputs():
    ...
    try:
        ...
        assert isinstance(e, ValueError) or isinstance(e, ZeroDivisionError), "LOWESS does not handle empty series."

    try:
        exponential_smoother(empty_series)
    except Exception as e:
        assert isinstance(e, ValueError) or isinstance(e, IndexError), "Exponential smoother does not handle empty series."
